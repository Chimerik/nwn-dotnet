...
using Ability = NWN.Enums.Ability;
using Alignment = NWN.Enums.Item.Property.Alignment;
using AlignmentGroup = NWN.Enums.Item.Property.AlignmentGroup;
using DamageType = NWN.Enums.Item.Property.DamageType;
using Feat = NWN.Enums.Item.Property.Feat;
using RacialType = NWN.Enums.Item.Property.RacialType;
using SpellSchool = NWN.Enums.SpellSchool;
...
namespace NWN
{
    public partial class NWScript
    {
        ...
        // ----------------------------------------------------------------------------
        public static void IPRemoveMatchingItemProperties(uint oItem, int nItemPropertyType, DurationType nItemPropertyDuration = DurationType.Temporary, int nItemPropertySubType = -1)
        {
            ItemProperty ip = GetFirstItemProperty(oItem);

            // valid ip?
            while (NWScript.GetIsItemPropertyValid(ip) != 0)
            {
                // same property type?
                if ((NWScript.GetItemPropertyType(ip) == nItemPropertyType))
                {
                    // same duration or duration ignored?
                    if (NWScript.GetItemPropertyDurationType(ip) == (int)nItemPropertyDuration || nItemPropertyDuration == DurationType.Invalid)
                    {
        ...
        /// </summary>
        public static ItemProperty ItemPropertyDecreaseAbility(Enums.Item.Property.Ability nAbility, int nModifier)
        {
