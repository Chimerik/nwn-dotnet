      ...
      public class FightingStyleSelectionWindow : PlayerWindow
      {
        ...
        public void CreateWindow(int fromLearnable)
        {
          ...
          if (player.oid.TryCreateNuiWindow(window, out NuiWindowToken tempToken, windowId))
          {
            ...
            if (fromLearnable == CustomSkill.BardCollegeDeLescrime)
              currentList = learnableDictionary.Values.Where(s => s is LearnableSkill ls && Utils.In(s.id, CustomSkill.FighterCombatStyleDuel, CustomSkill.FighterCombatStyleDualWield));
            else
            {
              currentList = learnableDictionary.Values.Where(s => s is LearnableSkill ls && ls.category == Category.FightingStyle && (!player.learnableSkills.ContainsKey(s.id)
