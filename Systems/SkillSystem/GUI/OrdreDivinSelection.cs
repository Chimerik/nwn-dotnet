...
namespace NWN.Systems
{
  public partial class PlayerSystem
  {
    ...
    {
      public class OrdreDivinSelectionWindow : PlayerWindow
      {
        ...
        public OrdreDivinSelectionWindow(Player player) : base(player)
        {
          windowId = "ordreDivinSelection";
        ...
        {
          if (player.learnableSkills.Any(s => s.Value.category == Category.OrdreDivin))
          {
            player.oid.LoginCreature.GetObjectVariable<PersistentVariableInt>("_IN_ORDRE_DIVIN_SELECTION").Delete();
            player.oid.SendServerMessage("Vous appartenez déjà à un ordre divin", ColorConstants.Orange);
            return;
          }

          player.oid.LoginCreature.GetObjectVariable<PersistentVariableInt>("_IN_ORDRE_DIVIN_SELECTION").Value = 1;
          ...
          window = new NuiWindow(rootColumn, "Choisissez votre ordre divin")
          {
            Geometry = geometry,
            Resizable = false,
            Collapsed = false,
          ...
          if (player.oid.TryCreateNuiWindow(window, out NuiWindowToken tempToken, windowId))
          {
            ...
            selectedItemDescription.SetBindValue(player.oid, nuiToken.Token, "Sélectionnez un ordre divin pour afficher ses détails.\n\nAttention, le choix est définitif.");
            ...
            currentList = learnableDictionary.Values.Where(s => s is LearnableSkill ls && ls.category == Category.OrdreDivin
