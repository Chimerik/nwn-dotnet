  ...
  public partial class PlayerSystem
  {
    public partial class Player
    {
      public class SkillBonusChoiceWindow : PlayerWindow
      {
        ...
        public SkillBonusChoiceWindow(Player player, int level, int source) : base(player)
        {
        ...
        public void CreateWindow(int level, int source)
        {
          ...
          window = new NuiWindow(rootColumn, $"{SkillSystem.learnableDictionary[source].name} - Choisissez une maîtrise de compétence")
          {
            Geometry = geometry,
            Resizable = false,
            Collapsed = false,
              ...
              switch (nuiEvent.ElementId)
              {
                case "validate":
                  ...
                  if (player.learnableSkills[sourceFeat].featOptions.Count < 0)
                    player.learnableSkills[sourceFeat].featOptions.Add(acquiredLevel, new int[] { selection, -1, -1 } );
                  else
                    player.learnableSkills[sourceFeat].featOptions[acquiredLevel][0] = selection;
