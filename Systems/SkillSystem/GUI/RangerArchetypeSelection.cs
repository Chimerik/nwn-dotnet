...
namespace NWN.Systems
{
  public partial class PlayerSystem
  {
    ...
    {
      public class RangerArchetypeSelectionWindow : PlayerWindow
      {
        ...
        public RangerArchetypeSelectionWindow(Player player) : base(player)
        {
          windowId = "rangerArchetypeSelection";
        ...
        {
          player.oid.LoginCreature.GetObjectVariable<PersistentVariableInt>("_IN_RANGER_ARCHETYPE_SELECTION").Value = 1;
          ...
          window = new NuiWindow(rootColumn, "Choisissez un archétype de rôdeur")
          {
            Geometry = geometry,
            Resizable = false,
            Collapsed = false,
          ...
          if (player.oid.TryCreateNuiWindow(window, out NuiWindowToken tempToken, windowId))
          {
            ...
            currentList = learnableDictionary.Values.Where(s => s is LearnableSkill ls && ls.category == Category.RangerArchetype).OrderBy(s => s.name);
            ...
            case NuiEventType.Click:
              ...
              switch(nuiEvent.ElementId)
              {
                ...
                case "validate":
                  ...
                  player.oid.SendServerMessage($"Vous avez choisi l'archétype de rôdeur : {StringUtils.ToWhitecolor(selectedLearnable.name)} !", ColorConstants.Orange);

                  CloseWindow();

                  if (!player.windows.TryGetValue("favoredEnemySelection", out var value)) player.windows.Add("favoredEnemySelection", new FavoredEnemySelectionWindow(player, selectedLearnable.id));
                  else ((FavoredEnemySelectionWindow)value).CreateWindow(selectedLearnable.id);
