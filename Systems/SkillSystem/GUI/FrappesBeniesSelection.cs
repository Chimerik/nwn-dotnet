...
namespace NWN.Systems
{
  public partial class PlayerSystem
  {
    ...
    {
      public class FrappesBeniesSelectionWindow : PlayerWindow
      {
        ...
        public FrappesBeniesSelectionWindow(Player player) : base(player)
        {
          windowId = "frappesBeniesSelection";
        ...
        {
          if (player.learnableSkills.Any(s => s.Value.category == Category.FrappesBenies))
          {
            player.oid.LoginCreature.GetObjectVariable<PersistentVariableInt>("_IN_FRAPPES_BENIES_SELECTION").Delete();
            player.oid.SendServerMessage("Vous avez déjà choisi votre frappe bénie", ColorConstants.Orange);
            return;
          }

          player.oid.LoginCreature.GetObjectVariable<PersistentVariableInt>("_IN_FRAPPES_BENIES_SELECTION").Value = 1;
          ...
          window = new NuiWindow(rootColumn, "Choisissez votre frappe bénie")
          {
            Geometry = geometry,
            Resizable = false,
            Collapsed = false,
          ...
          if (player.oid.TryCreateNuiWindow(window, out NuiWindowToken tempToken, windowId))
          {
            ...
            selectedItemDescription.SetBindValue(player.oid, nuiToken.Token, "Sélectionnez une frappe bénie pour afficher ses détails.\n\nAttention, le choix est définitif.");
            ...
            currentList = learnableDictionary.Values.Where(s => s is LearnableSkill ls && ls.category == Category.FrappesBenies
