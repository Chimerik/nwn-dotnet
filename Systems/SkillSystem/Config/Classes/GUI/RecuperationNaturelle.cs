...
namespace NWN.Systems
{
  public partial class PlayerSystem
  {
    ...
    {
      public class RecuperationNaturelleWindow : PlayerWindow
      {
        ...
        public RecuperationNaturelleWindow(Player player) : base(player)
        {
          windowId = "recuperationNaturelle";
        ...
        public void CreateWindow()
        {
          ...
          window = new NuiWindow(rootGroup, "Récupération Naturelle")
          {
            Geometry = geometry,
            Closable = true,
            Border = false,
          ...
          switch (nuiEvent.EventType)
          {
            case NuiEventType.Click:
              ...
              if (byte.TryParse(nuiEvent.ElementId.Split("_")[1], out byte level))
              {
                ...
                player.oid.LoginCreature.SetFeatRemainingUses((Feat)CustomSkill.DruideRecuperationNaturelle, (byte)(player.oid.LoginCreature.GetFeatRemainingUses((Feat)CustomSkill.DruideRecuperationNaturelle) - level));

                if(player.oid.LoginCreature.GetFeatRemainingUses((Feat)CustomSkill.DruideRecuperationNaturelle) < 1)
                  CloseWindow();

                player.oid.LoginCreature.ApplyEffect(EffectDuration.Instant, Effect.VisualEffect(VfxType.ImpMagicalVision));
                StringUtils.DisplayStringToAllPlayersNearTarget(player.oid.LoginCreature, $"{player.oid.LoginCreature.Name.ColorString(ColorConstants.Cyan)} - Récupération Naturelle", StringUtils.gold, true, true);
        ...
        private void SetLevelLayout()
        {
          ...
          var spellGainTable = NwClass.FromClassType(ClassType.Druid).SpellGainTable;
          int level = 0;

          foreach (var spellLevel in player.learnableSpells.Values.Where(s => s.currentLevel > 0&& s.learntFromClasses.Contains(CustomClass.Druid)).Select(s => NwSpell.FromSpellId(s.id).GetSpellLevelForClass(ClassType.Druid)).Distinct().Order())
          {
            if (spellLevel > 5)
              continue;
            ...
            if (spellLevel > 0 && spellLevel < 6)
            {
              ...
              else if (player.oid.LoginCreature.GetFeatRemainingUses((Feat)CustomSkill.DruideRecuperationNaturelle) < level)
