  ...
  public partial class FeatSystem
  {
    private static void FrappeBrutale(NwCreature caster, int featId)
    {
      if (caster.ActiveEffects.Any(e => e.Tag == EffectSystem.RecklessAttackEffectTag))
      {
        ...
        StringUtils.DisplayStringToAllPlayersNearTarget(caster, $"{caster.Name.ColorString(ColorConstants.Cyan)} - {StringUtils.ToWhitecolor(SkillSystem.learnableDictionary[featId].name)}", StringUtils.gold, true);
      }
      else
