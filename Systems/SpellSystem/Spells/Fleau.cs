  ...
  public partial class SpellSystem
  {
    public static List<NwGameObject> Fleau(NwGameObject oCaster, NwSpell spell, SpellEntry spellEntry, NwClass castingClass)
    {
      ...
      if (caster.IsLoginPlayerCharacter)
      {
        ...
        {
          targets.Add(caster.GetObjectVariable<LocalVariableObject<NwCreature>>($"_BANE_TARGET_{i}").Value);
          objectTargets.Add(caster.GetObjectVariable<LocalVariableObject<NwCreature>>($"_BANE_TARGET_{i}").Value);
          caster.GetObjectVariable<LocalVariableObject<NwCreature>>($"_BANE_TARGET_{i}").Delete();
        }

        caster.GetObjectVariable<LocalVariableInt>("_BANE_TARGETS").Delete();
      }
      else
      {
