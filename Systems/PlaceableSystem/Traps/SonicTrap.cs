  ...
  public partial class PlaceableSystem
  {
    public static void SonicTrap(NwGameObject trap, TrapEntry entry)
    {
      foreach (NwCreature creature in trap.Location.GetObjectsInShapeByType<NwCreature>(Shape.Sphere, entry.aoeSize, false))
      {
        ...
        int advantage = CreatureUtils.GetCreatureSavingThrowAdvantage(creature, Ability.Wisdom, null, SpellConfig.SpellEffectType.Trap, trap);
        ...
        TrapUtils.SendSavingThrowFeedbackMessage(creature, feedback.saveRoll, feedback.proficiencyBonus, advantage, entry.baseDC, totalSave, saveResult, Ability.Wisdom);
