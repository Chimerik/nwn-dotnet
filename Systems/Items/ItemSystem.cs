...
namespace NWN.Systems
{
    ...
    public class ItemSystem
    {
        ...
        private void OnItemEquipBefore(ItemEvents.OnItemEquipBefore onItemEquip)
        {
            ...
            if (oUnequip.IsValid && ObjectPlugin.CheckFit(oPC, (int)oUnequip.BaseItemType) == 0)
            {
        ...
        private void OnAcquireItem(ModuleEvents.OnAcquireItem onAcquireItem)
        {
            ...
            if (oItem.Tag == "item_pccorpse" && oAcquiredFrom.Tag == "pccorpse_bodybag")
            {
                ...
                NwCreature oCorpse = ((uint)NwObject.FindObjectsWithTag("pccorpse").Where(c => c.GetLocalVariable<int>("_PC_ID").Value == oItem.GetLocalVariable<int>("_PC_ID")).FirstOrDefault()).ToNwObject<NwCreature>();
                oCorpse.Destroy();
        ...
        private void OnUnacquireItem(ModuleEvents.OnUnacquireItem onUnacquireItem)
        {
            ...
            if (!oPC.IsValid || !oItem.IsValid)
                return;

            NwGameObject oGivenTo = oItem.Possessor;

            if (oItem.Tag == "item_pccorpse" && !oGivenTo.IsValid) // signifie que l'item a été drop au sol et pas donné à un autre PJ ou mis dans un placeable
            {
