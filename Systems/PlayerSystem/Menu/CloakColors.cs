...
namespace NWN.Systems
{
  public partial class PlayerSystem
  {
    ...
    {
      public void CreateCloakColorsWindow(NwItem item)
      {
        string windowId = "cloakColorsModifier";
        ...
        NuiRect windowRectangle = windowRectangles.ContainsKey(windowId) ? windowRectangles[windowId] : new NuiRect(oid.GetDeviceProperty(PlayerDeviceProperty.GuiWidth) * 0.01f, oid.GetDeviceProperty(PlayerDeviceProperty.GuiHeight) * 0.01f, oid.GetDeviceProperty(PlayerDeviceProperty.GuiWidth) / 4 + 50, oid.GetDeviceProperty(PlayerDeviceProperty.GuiHeight) / 2);
        ...
        oid.OnNuiEvent -= HandleCloakColorsEvents;
        oid.OnNuiEvent += HandleCloakColorsEvents;
      ...
      {
        if (nuiEvent.Player.NuiGetWindowId(nuiEvent.WindowToken) != "cloakColorsModifier" || !Players.TryGetValue(nuiEvent.Player.LoginCreature, out Player player))
          return;
          ...
          case NuiEventType.Watch:
            ...
            if (nuiEvent.ElementId == "channelSelection")
            {
              ...
              for (int i = 0; i < 4; i++)
              {
                ...
                row.Children = rowChildren;
                paletteGroup.Layout = row;
                paletteGroup.SetLayout(player.oid, nuiEvent.WindowToken, paletteGroup);
              }
            }
