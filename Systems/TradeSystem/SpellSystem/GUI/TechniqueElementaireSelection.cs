...
namespace NWN.Systems
{
  public partial class PlayerSystem
  {
    ...
    {
      public class TechniqueElementaireSelectionWindow : PlayerWindow
      {
        ...
        public TechniqueElementaireSelectionWindow(Player player, int nbTechs) : base(player)
        {
          windowId = "techniqueElementaireSelection";
        ...
        public async void CreateWindow(int nbTechs)
        {
          ...
          window = new NuiWindow(rootColumn, $"Choix de {nbTechs} techniques élémentaires")
          {
            Geometry = geometry,
            Resizable = false,
            Collapsed = false,
          ...
          if (player.oid.TryCreateNuiWindow(window, out NuiWindowToken tempToken, windowId))
          {
            ...
            player.oid.LoginCreature.GetObjectVariable<PersistentVariableInt>("_IN_TECHNIQUE_ELEMENTAIRE_SELECTION").Value = nbTechs;            
          }
          else
              ...
              switch (nuiEvent.ElementId)
              {
                ...
                case "validate":
                  ...
                  foreach (var tech in acquiredTechs)
                  {
                    ...
                    player.oid.SendServerMessage($"Vous apprenez la technique élémentaire {StringUtils.ToWhitecolor(tech.name)}", ColorConstants.Orange);
                  }

                  player.oid.LoginCreature.GetObjectVariable<PersistentVariableInt>("_IN_TECHNIQUE_ELEMENTAIRE_SELECTION").Delete();
        ...
        private void InitTechsBinding()
        {
          ...
          && skill.category == Category.TechniqueElementaire 
          && skill.minLevel <= player.oid.LoginCreature.GetClassInfo((ClassType)CustomClass.Monk).Level).OrderBy(s => s.name).Cast<LearnableSkill>())
          {
